...
my $SERVICES = new Libs::Services($CONF, $LOG, $VALIDATE, $DBI, $USER, $WEB, $ENTITIES);
$LOG->delay("create_object_services", "Created object SERVICES");

$LOG->delay("create_object_sites");
my $SITES = new Libs::Sites($CONF, $LOG, $VALIDATE, $DBI, $USER, $WEB, $ENTITIES, $SERVICES);
$LOG->delay("create_object_sites", "Created object SITES");

$LOG->delay("create_object_pages");
my $PAGES = new Libs::Pages($CONF, $LOG, $VALIDATE, $DBI, $USER, $WEB, $ENTITIES, $SERVICES, $SITES);
...
$CGITempFile::TMPDIRECTORY = $CONF->getValue("pwe", "home", "/tmp/") . $CONF->getValue("pwe", "cgi_tmpdirectory", "cgitemp/");
...
while (my $CGI = CGI::Fast->new()) {
    ...
    my $env  = \%ENV;
    my $sid  = $CGI->cookie($CONF->getValue("http", "cookie_name", "UNKNOWN_SESSION_NAME"));
    my $page = (defined($CGI->param('page')) ? $CGI->param('page') : $CONF->getValue("pwe", "default_page", 'default'));
    my $func = (defined($CGI->param('func')) ? $CGI->param('func') : $CONF->getValue("pwe", "default_func", 'default'));
    my $opt  = (defined($CGI->param('opt')) ? $CGI->param('opt') : $CONF->getValue("pwe", "enable_opt", 0));
