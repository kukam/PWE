package Libs::jQuery::File::Upload;
...
sub upload_dir {
    ...
    if (!(defined $self->{upload_dir})) {
        my $script_name = $CONF->getValue('pwe', 'cgi_script_name', "unknown_script_name");
        $self->{upload_dir} = abs_path($script_name);
        $self->{upload_dir} =~ s/(.*)\/.*/$1/;
...
sub _validate_file {
    my $self = shift;
    return undef
      unless $self->_validate_max_file_size
      and $self->_validate_min_file_size
      and $self->_validate_accept_file_types
      and $self->_validate_reject_file_types
      and $self->_validate_max_width
      and $self->_validate_min_width
      and $self->_validate_max_height
      and $self->_validate_min_height
      and $self->_validate_max_number_of_files;
