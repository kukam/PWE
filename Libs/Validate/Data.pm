...
sub is_integer {
	...
	# TODO : Nepouzivam, protoze pokud bych tutot cas pouzil, perl by mi zacal vracet decimal cisla ve spatnem tvaru
	# MOZNO VYZKOUSET S COMMAND LINY:
	# $ LC_NUMERIC=C perl -e 'use POSIX; POSIX::strtod(123); my $x = (15/100); print "$x";'
	# $ LC_NUMERIC=cs_CZ perl -e 'use POSIX; POSIX::strtod(123); my $x = (15/100); print "$x";'
	#my($int, $leftover) = POSIX::strtod($value);
	#return if $leftover;

	return if (int($int) != $int);
